...
2. **Build locally** by running `./gradlew clean build`. If there are any javac errors,
   fix them and rerun this check to verify your changes.
...
6. **Create or update PR**:
   ...
   - Labels — add any that are relevant:
     - `bug`, `build`, `ci/cd`, `documentation`, `enhancement`
     - Only add a `documentation` label if the PR is *only* doc changes
